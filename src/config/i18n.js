...
const TERMINOLOGY_PLAYGROUND_I18N = {
  ...
  terminology_playground_terms_helper: terminologyPlaygroundText(
    "术语键按正则语义解析：含正则元字符（如 .）需转义，例如字面点号写为 Dr\\.whob；译文留空 = 不翻译（保留原文，只写术语键不加尾部逗号）；重复的术语键只保留首次；术语按正则语义在原文任意位置匹配（与生产一致），建议长度长的术语写在前面。简单英文词若只需匹配独立单词，请显式添加边界（如 \\bAPI\\b）；该写法不适用于中文或 C++、.NET 等符号术语。",
    "Term keys are parsed as regex: escape regex metacharacters (e.g. a literal dot as Dr\\.whob). An empty value keeps the original (write the key only, without a trailing comma); duplicate keys keep the first; terms match anywhere in the source text by regex semantics (identical to production), so put longer terms first. To match a simple English word only when it stands alone, add explicit boundaries (for example \\bAPI\\b); this does not apply to Chinese or symbol terms such as C++ and .NET.",
    "術語鍵依正則語意解析：含正則元字元（如 .）需跳脫，例如字面點號寫為 Dr\\.whob；譯文留空 = 不翻譯（保留原文，只寫術語鍵不加尾部逗號）；重複的術語鍵只保留首次；術語依正則語意在原文任意位置匹配（與生產一致），建議長度較長的術語寫在前面。簡單英文詞若只需匹配獨立單詞，請明確加入邊界（如 \\bAPI\\b）；此寫法不適用於中文或 C++、.NET 等符號術語。",
    "用語 key は正規表現として解析されます。正規表現メタ文字（例：.）はエスケープが必要です（例：リテラルのドットは Dr\\.whob）。value を空にすると翻訳しません（原文保持、カンマなしで key のみ記述）。重複 key は最初のみ有効です。用語は正規表現の意味で原文の任意の位置に一致します（本番と同一）。長い用語ほど先に書くことを推奨します。単純な英単語だけを独立した語として一致させる場合は、境界を明示してください（例：\\bAPI\\b）。これは中国語や C++、.NET などの記号を含む用語には適用されません。",
    "용어 key 는 정규식으로 해석됩니다. 정규식 메타 문자(예: .)는 이스케이프가 필요합니다(예: 리터럴 점은 Dr\\.whob). value 가 비어 있으면 번역하지 않고 원문을 유지합니다(key 만 작성, 끝에 쉼표 없음). 중복 key 는 첫 번째만 유지됩니다. 용어는 정규식 의미로 원문의 어느 위치에서나 일치합니다(프로덕션과 동일). 길이가 긴 용어를 앞에 쓰는 것을 권장합니다. 간단한 영어 단어를 독립된 단어로만 일치시키려면 경계를 명시하세요(예: \\bAPI\\b). 이 방식은 중국어나 C++, .NET 같은 기호 용어에는 적용되지 않습니다.",
    "Terim anahtarları normal ifade olarak ayrıştırılır: normal ifade meta karakterlerini (ör. .) kaçışlayın (ör. değişmez nokta Dr\\.whob). Boş value = çevrilmez (orijinal korunur, virgülsüz yalnızca key yazın); aynı key'den yalnızca ilki tutulur. Terimler normal ifade anlamına göre kaynak metnin herhangi bir yerinde eşleşir (üretimle aynı); daha uzun terimleri önce yazın. Basit bir İngilizce sözcüğü yalnızca bağımsız olduğunda eşleştirmek için açık sınırlar ekleyin (ör. \\bAPI\\b); bu yöntem Çince veya C++ ve .NET gibi simgeli terimler için geçerli değildir.",
    "Khóa thuật ngữ được phân tích cú pháp như regex: hãy thoát các ký tự đặc biệt regex (ví dụ dấu chấm chữ thường viết là Dr\\.whob). value trống = không dịch (giữ nguyên văn bản, chỉ viết key không thêm dấu phẩy cuối); key trùng chỉ giữ lần đầu. Thuật ngữ khớp ở bất kỳ vị trí nào trong văn bản gốc theo ngữ nghĩa regex (giống bản chính thức); nên đặt thuật ngữ dài hơn ở trước. Để chỉ khớp một từ tiếng Anh đơn giản khi nó đứng độc lập, hãy thêm ranh giới rõ ràng (ví dụ \\bAPI\\b); cách này không áp dụng cho tiếng Trung hoặc thuật ngữ có ký hiệu như C++ và .NET."
  ),
...
export const I18N = {
  ...
  preset_prompt_nobatch_translation_concise: {
    zh: `单句翻译 (Concise)`,
    en: `Single-sentence translation (Concise)`,
    zh_TW: `單句翻譯 (Concise)`,
    ja: `単文翻訳 (Concise)`,
    ko: `단일 문장 번역 (Concise)`,
    tr: `Tek Cümle Çevirisi (Concise)`,
    vi: "Dịch từng câu (Concise)",
  },
  preset_prompt_nobatch_translation: {
    zh: `单句翻译 (Legacy)`,
    en: `Single-sentence translation (Legacy)`,
    zh_TW: `單句翻譯 (Legacy)`,
    ja: `単文翻訳 (Legacy)`,
    ko: `단일 문장 번역 (Legacy)`,
    tr: `Tek Cümle Çevirisi (Legacy)`,
    vi: "Dịch từng câu (Legacy)",
  },
  ...
  sync_warn: {
    zh: `1、涉及隐私数据的同步请谨慎选择第三方同步服务，建议自行搭建 kiss-worker 或 WebDAV 服务。`,
    en: `1. When synchronizing data that involves privacy, please be cautious about choosing third-party sync services. It is recommended to set up your own sync service using kiss-worker or WebDAV.`,
    zh_TW: `1. 同步涉及隱私資料時，請謹慎選擇第三方同步服務；建議自行架設 kiss-worker 或 WebDAV 服務。`,
    ja: `1. プライバシーに関わるデータを同期する場合、サードパーティの同期サービスは慎重に選択してください。kiss-worker や WebDAV サービスを自己ホスティングすることをお勧めします。`,
    ko: `1. 개인정보가 포함된 데이터를 동기화할 경우, 타사 동기화 서비스 선택에 신중을 기하십시오. 자체 kiss-worker 또는 WebDAV 서비스를 구축하는 것을 권장합니다.`,
    tr: `1. Gizlilikle ilgili verileri senkronize ederken, üçüncü taraf senkronizasyon hizmetlerini seçerken lütfen dikkatli olun. KISS-Worker veya WebDAV kullanarak kendi senkronizasyon hizmetinizi kurmanız önerilir`,
    vi: "1. When synchronizing data that involves privacy, please be cautious about choosing third-party sync services. It is recommended to set up your own sync service using kiss-worker or WebDAV.",
  },
  sync_warn_gist: {
    zh: `3、GitHub Gist 不会出现在公开搜索或发现页面中，但任何拥有该链接的人都可以查看内容。`,
    en: `3. GitHub Gists do not appear in public searches or discovery pages, but anyone with the link can view the content.`,
    zh_TW: `3. GitHub Gist 不會出現在公開搜尋或探索頁面中，但任何擁有該連結的人都可以查看內容。`,
    ja: `3. GitHub Gist は公開検索や発見ページには表示されませんが、リンクを持っている人は誰でも内容を閲覧できます。`,
    ko: `3. GitHub Gist는 공개 검색이나 탐색 페이지에 표시되지 않지만, 링크를 가진 사람은 누구나 내용을 볼 수 있습니다.`,
    tr: `3. GitHub Gist'leri herkese açık aramalarda veya keşif sayfalarında görünmez, ancak bağlantıya sahip herkes içeriği görüntüleyebilir.`,
    vi: "3. GitHub Gists do not appear in public searches or discovery pages, but anyone with the link can view the content.",
  },
  sync_warn_2: {
    zh: `2、如果服务器存在其他客户端同步的数据，第一次同步将直接覆盖本地配置，后面则根据修改时间，新的覆盖旧的。`,
    en: `2. If the server has data synchronized by other clients, the first synchronization will directly overwrite the local configuration, and later, according to the modification time, the new one will overwrite the old one.`,
    zh_TW: `2. 若伺服器上存在其他用戶端同步的資料，第一次同步會直接覆蓋本機設定；之後則依修改時間，由新的覆蓋舊的。`,
    ja: `2. サーバーに他のクライアントによって同期されたデータがある場合、最初の同期はローカル設定を直接上書きし、その後は変更時間に応じて新しいものが古いものを上書きします。`,
    ko: `2. 서버에 다른 클라이언트가 동기화한 데이터가 있는 경우, 첫 번째 동기화는 로컬 구성을 직접 덮어쓰며, 이후에는 수정 시간에 따라 새 항목이 기존 항목을 덮어씁니다.`,
    tr: `2. Eğer sunucuda diğer istemciler tarafından senkronize edilen veriler varsa, ilk senkronizasyon doğrudan yerel yapılandırmanın üzerine yazılır ve daha sonra, değişiklik zamanına göre, yenisi eskisinin üzerine yazılır`,
    vi: "2. If the server has data synchronized by other clients, the first synchronization will directly overwrite the local configuration, and later, according to the modification time, the new one will overwrite the old one.",
  },
  ...
  about_api_2: {
    zh: `2、大部分AI接口都与OpenAI兼容，因此选择OpenAI类型即可。`,
    en: `2. Most AI interfaces are compatible with OpenAI, so you can simply select the OpenAI type.`,
    zh_TW: `2. 大部分的 AI 介面都與 OpenAI 相容，因此選擇 OpenAI 類型即可。`,
    ja: `2. ほとんどの AI インターフェースは OpenAI と互換性があるため、OpenAI タイプを選択すれば問題ありません。`,
    ko: `2. 대부분의 AI 인터페이스는 OpenAI와 호환되므로 OpenAI 유형을 선택하면 됩니다.`,
    tr: `2. Çoğu AI arayüzü OpenAI ile uyumludur, bu nedenle OpenAI türünü seçmeniz yeterlidir.`,
    vi: "2. Most AI interfaces are compatible with OpenAI, so you can simply select the OpenAI type.",
  },
  about_api_3: {
    zh: `3、对于一些小模型（如 Hy-MT2-1.8B），建议优先选用 单句翻译 (Concise) 或 聚合翻译（LINE）。`,
    en: `3. For smaller models (e.g. Hy-MT2-1.8B), it is recommended to prioritize Single-sentence Translation (Concise) or Batch Translation (LINE).`,
    zh_TW: `3. 對於一些小型模型（如 Hy-MT2-1.8B），建議優先選用 單句翻譯 (Concise) 或 聚合翻譯（LINE）。`,
    ja: `3. 一部の小型モデル（例: Hy-MT2-1.8B）では、「単文翻訳（Concise）」または「一括翻訳（LINE）」を優先して選択することをお勧めします。`,
    ko: `3. 일부 소형 모델(예: Hy-MT2-1.8B)의 경우 '단문 번역 (Concise)' 또는 '일괄 번역 (LINE)'을 우선 사용하는 것을 권장합니다.`,
    tr: `3. Bazı küçük modeller (ör. Hy-MT2-1.8B) için Tek Cümle Çeviri (Concise) veya Toplu Çeviri (LINE) tercih edilmesi önerilir.`,
    vi: "3. Đối với một số mô hình nhỏ (ví dụ Hy-MT2-1.8B), nên ưu tiên chọn Dịch từng câu (Concise) hoặc Dịch hàng loạt (LINE).",
  },
  about_api_4: {
    zh: `4、理论上，所有翻译接口，都可以通过自定义接口 (Custom) 的形式使用。`,
    en: `4. In theory, all translation interfaces can be used by configuring them as a custom interface.`,
    zh_TW: `4. 理論上，所有翻譯介面都可以透過自訂介面（Custom）的方式來使用。`,
    ja: `4. 理論的には、すべての翻訳インターフェースはカスタム（Custom）インターフェースとして設定することで利用できます。`,
    ko: `4. 이론적으로 모든 번역 인터페이스는 커스텀(Custom) 인터페이스로 설정하여 사용할 수 있습니다.`,
    tr: `4. Teorik olarak, tüm çeviri arayüzleri özel bir arayüz olarak yapılandırılarak kullanılabilir.`,
    vi: "4. In theory, all translation interfaces can be used by configuring them as a custom interface.",
  },
  about_api_proxy: {
    zh: `查看自建一个翻译接口代理`,
  ...
  check_update_helper: {
    zh: `进入设置页面时自动检查是否有新版本`,
    en: `Automatically check for new versions when opening settings`,
    zh_TW: `進入設定頁面時自動檢查是否有新版本`,
    ja: `設定ページを開いたときに新しいバージョンを自動的に確認する`,
    ko: `설정 페이지를 열 때 새 버전을 자동으로 확인합니다`,
    tr: `Ayarlar açıldığında yeni sürümleri otomatik olarak denetle`,
    vi: `Tự động kiểm tra phiên bản mới khi mở trang cài đặt`,
  },
